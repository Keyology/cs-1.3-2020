...
class BinaryTreeNode(object):
    ...
    def is_leaf(self):
        ...
        if self.data is not None:  
            if self.left is not  None or self.right is not None: return False
        return True
    ...
    def is_branch(self):
        ...
        if self.right is not None or self.left is not None: return True
        return False

    def height(self):
        """Return the height of this node (the number of edges on the longest
        downward path from this node to a descendant leaf node).
        TODO: Best and worst case running time: O(n) because we have to traverse down the tree"""
        
        # TODO: Check if left child has a value and if so calculate its height
        if self is None:return 0
        
        left_height, right_height = 0, 0
        if self.left is not None:  left_height = self.left.height() + 1
        if self.right is not None: right_height = self.right.height() + 1
...
class BinarySearchTree(object):
    ...
    def height(self):
        ...
        Best and worst case running time: O(n) where h is the height of
        the tree."""
    ...
    def _traverse_in_order_recursive(self, node, visit):
        ...
        Memory usage: O(n) based off the height of the tree."""
